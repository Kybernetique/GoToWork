...
{
    /// <summary>
    /// Логика взаимодействия для WindowShiftReport.xaml
    /// </summary>
    public partial class WindowShiftReport : Window
    {
        [Dependency]
        ...
        ShiftReportLogic logicR;

        List<ProductViewModel> list = new List<ProductViewModel>();

        public WindowShiftReport(ShiftReportLogic _logicR, ProductLogic _logicP)
        {
            InitializeComponent();
            logicR = _logicR;
            logicP = _logicP;
        ...
        private void Add_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                if (!list.Contains(logicP.Read(new ProductBindingModel { Id = window.Id })[0]))
                {
                    list.Add(logicP.Read(new ProductBindingModel { Id = window.Id })[0]);
                    LoadData();
                }
            }
        }
