...
{
    /// <summary>
    /// Логика взаимодействия для Certificates.xaml
    /// </summary>
    public partial class Certificates : Window
    {
        ...
        public Certificates(CertificateLogic logic)
        {
            InitializeComponent();
            _logic = logic;
        }
        ...
        public void LoadData()
        {
            try
            {
                var list = _logic.Read(new CertificateBindingModel { EngineerId = _engineerId });
                if (list != null)
                {
        ...
        {
            var window = Container.Resolve<Certificate>();
        ...
        private void Delete_Click(object sender, RoutedEventArgs e)
        {
            if (DataGridView.SelectedIndex != -1)
            {
               ...
               MessageBoxImage.Question);
                ...
                {
                    CertificateViewModel certificate = (CertificateViewModel)DataGridView.SelectedCells[0].Item;
                    int id = Convert.ToInt32(certificate.Id);
                    try
                    {
                        _logic.Delete(new CertificateBindingModel { Id = id });
                    }
                    catch (Exception ex)
                    {
                        MessageBox.Show(ex.Message, "Ошибка", MessageBoxButton.OK,
        ...
        private void Change_Click(object sender, RoutedEventArgs e)
        {
            if (DataGridView.SelectedIndex != -1)
            {
                var window = Container.Resolve<Certificate>();
                CertificateViewModel certificate = (CertificateViewModel)DataGridView.SelectedCells[0].Item;
                window.Id = Convert.ToInt32(certificate.Id);
                window._engineerId = _engineerId;
                window.ShowDialog();
                if (window.DialogResult == true)
                {
